...
export function UserInfoDialog({
  ...
  useEffect(() => {
    if (open && userId) {
      const scheduledSequence = ++requestSequenceRef.current
      queueMicrotask(() => {
        if (scheduledSequence === requestSequenceRef.current) {
  ...
  if (isLoading) {
    ...
    dialogContent = (
      <div className='space-y-4 py-4'>
        ...
        {/* Promotion usage */}
        {userInfo.free_usage_limit !== undefined && (
          <div className='grid grid-cols-2 gap-4'>
            <InfoItem
              label={t('Free Usage Remaining')}
              value={formatCompactNumber(userInfo.free_usage_remaining ?? 0)}
            />
            <InfoItem
              label={t('Free Usage Limit')}
              value={formatCompactNumber(userInfo.free_usage_limit)}
            />
            <InfoItem
              label={t('Free Usage Used')}
              value={formatCompactNumber(userInfo.free_usage_used ?? 0)}
            />
          </div>
        )}
