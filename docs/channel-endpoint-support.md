...
| 24 | Gemini | Chat、Msg、Resp、Gemini | Emb | ImgG（Imagen、已配置的 Gemini 图片模型） | Gemini/Veo Video、异步图像（配置） | Gemini 图片模型通过 `generateContent` 返回图片；ImgG 显式比例/分辨率优先并按模型能力校验；无 Compact、ImgE、Audio 和 Rank |
...
| 41 | Vertex AI | Chat、Msg、Gemini | - | ImgG（Imagen、已配置的 Gemini 图片模型） | Vertex/Veo Video、异步图像（配置） | Gemini 图片模型通过 `generateContent` 返回图片；ImgG 与 Gemini 共用显式比例/分辨率转换；根据模型进入 Claude、Gemini 或 Open-source 模式；无 Responses、Embedding 和 Audio |
