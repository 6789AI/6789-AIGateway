...
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | OpenAI | Chat、Comp、Msg、Resp、Compact、Gemini、Mod | Emb、Rank（透传） | ImgG、ImgE、TTS、STT | RT、OpenAI Video | 官方上游是否实现 Rank 由上游决定；支持完整 OpenAI 视频任务 |
| 2 | MjProxy | - | - | - | Midjourney | 仅使用 `/mj` 专用任务路由 |
| 3 | Azure | Chat、Comp、Msg、Resp、Compact、Gemini、Mod | Emb、Rank（透传） | ImgG、ImgE、TTS、STT（条件） | RT | 使用 Azure deployment 和 API version URL；无视频任务适配器 |
| 4 | Ollama | Chat、Comp、Msg | Emb | - | - | 分别映射到 `/api/chat`、`/api/generate`、`/api/embed` |
| 5 | MjProxyPlus | - | - | - | Midjourney | 仅使用 `/mj` 专用任务路由 |
| 7 | OhMyGPT | OpenAI 兼容端点（透传，含 Compact） | 透传 | 透传 | RT（透传） | 使用 OpenAI 适配器；实际能力由上游决定；无视频任务 |
| 8 | Custom | OpenAI 兼容端点（条件） | 条件 | 条件 | RT（条件） | 所有请求发送到填写的同一个完整 URL，不自动拼接入口路径；无视频任务 |
...
| 17 | Ali | Chat、Comp、Msg（条件）、Resp | Emb、Rank | ImgG、ImgE | Ali Video、异步图像 | Anthropic Messages 仅配置匹配模型；不支持 Compact 和 Audio |
| 18 | Xunfei | Chat | - | - | - | 使用讯飞专用 WebSocket 请求链 |
| 19 | 360 | OpenAI 兼容端点（透传，含 Compact） | 透传 | 透传 | RT（透传） | 实际能力由上游决定；无视频任务 |
| 20 | OpenRouter | OpenAI 兼容端点（透传，不含 Compact） | 透传 | 透传 | RT（透传） | `APITypeOpenRouter` 不在 Compact 白名单；无视频任务 |
| 22 | FastGPT | OpenAI 兼容端点（透传，含 Compact） | 透传 | 透传 | RT（透传） | 实际能力由上游决定；无视频任务 |
| 23 | Tencent | Chat | - | - | - | 腾讯混元签名请求链 |
| 24 | Gemini | Chat、Msg、Resp、Gemini | Emb | ImgG（仅 Imagen） | Gemini/Veo Video | 无 Compact、ImgE、Audio 和 Rank |
| 25 | Moonshot | Chat、Comp、Msg | Emb、Rank | - | - | Responses 和 Audio 未实现 |
| 26 | Zhipu V4 | Chat、Msg | Emb | ImgG | - | 无 Responses、Audio 和 Rank |
| 27 | Perplexity | Chat、Msg、Resp | - | - | - | 无 Compact 及其他媒体端点 |
| 31 | LingYiWanWu | OpenAI 兼容端点（透传，含 Compact） | 透传 | 透传 | RT（透传） | 实际能力由上游决定；无视频任务 |
| 33 | AWS | Chat、Msg | - | - | - | Bedrock Claude/Nova 转换；无 Responses、Embedding 和任务端点 |
| 34 | Cohere | Chat | Rank | - | - | 无 Embedding、Responses 和媒体端点 |
| 35 | MiniMax | Chat、Msg | - | ImgG、TTS | MiniMax/Hailuo Video | Embedding 有请求函数但 URL 构造不支持，因此不计为支持 |
...
| 40 | SiliconFlow | Chat、Comp、Msg | Emb、Rank | ImgG、ImgE、TTS、STT | RT（透传） | Responses 未实现；实时能力由上游兼容性决定 |
| 41 | Vertex AI | Chat、Msg、Gemini | - | ImgG（仅 Imagen） | Vertex/Veo Video | 根据模型进入 Claude、Gemini 或 Open-source 模式；无 Responses、Embedding 和 Audio |
...
| 45 | VolcEngine | Chat、Msg、Resp | Emb | ImgG、TTS | Doubao Video | ImgE 转换未完成；Rank 请求转换为空，不计为支持 |
| 46 | Baidu V2 | Chat | - | - | - | URL 构造器虽列出 Emb/Img/Rank，但默认转换器未实现 |
| 47 | Xinference | OpenAI 兼容端点（透传，不含 Compact） | 透传 | 透传 | RT（透传） | `APITypeXinference` 不在 Compact 白名单；无视频任务 |
| 48 | xAI | Chat、Resp | - | ImgG、ImgE | - | 无 Compact、Messages、Embedding、Audio 和 Video |
| 49 | Coze | Chat | - | - | - | 使用 Coze chat 创建和轮询流程 |
| 50 | Kling | - | - | - | Kling Video | 支持通用视频路由及 `/kling/v1` 专用路由 |
| 51 | Jimeng | - | - | ImgG | Jimeng Video | 同步适配器面向图像；视频另走任务适配器和 `/jimeng/` 路由 |
...
| 56 | Replicate | - | - | ImgG、ImgE | - | 其他转换器明确未实现 |
| 57 | ChatGPT Subscription (Codex) | Resp、Compact、Alpha | - | - | - | 明确拒绝 Chat、Msg、Gemini、Embedding、Image、Audio 和 Rank |
| 58 | Advanced Custom | 按 route/converter 配置 | 按配置 | 按配置 | RT（按配置） | 可覆盖已注册同步路由和 Alpha；无视频 TaskAdaptor |
| 59 | Sub2API | Chat、Comp、Msg、Resp、Compact、Gemini、Mod、Alpha | Emb | ImgG、ImgE | - | 继承 New API 适配器；Audio、Rank、RT 和 Video 不支持 |
| 60 | New API | Chat、Comp、Msg、Resp、Compact、Gemini、Mod、Alpha | Emb | ImgG、ImgE | - | 适配器明确拒绝 Audio 和 Rank；无 RT 和 Video |
...
| `POST /v1/images/generations`，请求 `async=true` 或 Header `Prefer: respond-async` | Ali；以及配置为 Grsai 异步图像的特殊渠道 | 异步图像提交仍使用标准图像入口 |
| `GET /v1/images/generations/:task_id` | 异步图像任务 | 查询异步图像任务 |
