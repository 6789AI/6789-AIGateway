...
func TestValidateMultipartDirectPreservesNumericSeconds(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := strings.NewReader(`{"model":"minimax-h3","prompt":"animate","seconds":8}`)
	...
	storedReq, err := GetTaskRequest(context)
	require.NoError(t, err)
	assert.Equal(t, "8", storedReq.Seconds)
}
...
func TestTaskDurationBounds(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			body: `{"model":"minimax-h3","prompt":"a cat","seconds":8}`,
		},
	}
